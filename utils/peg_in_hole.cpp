...
int main(int argc, char** argv) {
  ...
  // *************** SCANSIONE FILE PUNTI DESIDERATI
   FILE* file2;
   file2 = fopen("/home/labarea-franka/libfranka/unibas_peginhole/resources/holes.txt", "r");
   Eigen::MatrixXd pHoles;
   char stringa[80];
   int res;
   int i = 0;
   int j = 0;
   ...
   std::vector<Eigen::Matrix3d> Rholes_vec(numPti); 
   pHoles.resize(numPti, 3);
   Eigen::Matrix3d Rd;
   ...
   cout << pHoles << endl;
   for(unsigned int k=0; k<Rholes_vec.size(); k++){
     cout << Rholes_vec[k] << endl;
   }

   std::cout << "Ci sono " << Rholes_vec.size() << " punti:\n" << pHoles << endl;
  
   cout << "Scegli verso quale vuoi andare? (da 1 a "  << Rholes_vec.size() << ")" << endl;
   int hole_choosen;
   cin >> hole_choosen;  
  cout << "Foro: " << hole_choosen << endl; 
  ...
  try{
    ...
    pHole << pHoles(hole_choosen-1, 0), pHoles(hole_choosen-1, 1), pHoles(hole_choosen-1, 2);
    R_temp = Rholes_vec[hole_choosen-1];
    cout << R_temp << endl;
    nH << R_temp(0,2), R_temp(1,2), R_temp(0,2);
    cout << nH << endl;
  ...
  //
    cout << "SONO ARRIVATA QUI: PREMI PER ANDARE AVANTI" << endl;
  cin.ignore();
  return 0;
