...
int main(int argc, char** argv) {
    ...
    FILE* file_i;
    file_i = fopen("/home/labarea-franka/libfranka/unibas_peginhole/resources/input/pos2Dic_rB.txt", "r");
    //file_i = fopen("/home/labarea-franka/libfranka/unibas_peginhole/resources/puntiScan.txt", "r");
    //file_i = fopen("/home/labarea-franka/libfranka/build/unibas_peginhole/data/input/18_novembre/handEyeCalibration/puntiRobot2.txt", "r");
    Eigen::MatrixXd P;
    char stringa[80];
    int res;
    int i = 1;
    ...
    try{
        ...
        file_o = fopen("/home/labarea-franka/libfranka/unibas_peginhole/resources/puntiSalv_franka.txt", "a");
        file_o_fk = fopen("/home/labarea-franka/libfranka/unibas_peginhole/resources/puntiSalv_fk.txt", "a");
        int i=0;
        while(i < num_seg ){
            Eigen::Vector3d point_i(P(i+1,0), P(i+1,1), P(i+1,2));
            move(robot_ip, point_i, Rf_vec[i+1], angles, 0, tempo, vi_tot, vf_tot);
            //detectAndSave(true, result_l, result_r, net, pipe, i+1);
            i++;
        }
        ...
        //matlab::data::TypedArray<double> const arg = factory.createScalar<int32_t>(3);
        std::vector<matlab::data::Array> args({ factory.createScalar<int32_t>(3)});    

        matlab::data::Array result = matlabPtr->feval(u"reconstruct_and_find_holes", args);
