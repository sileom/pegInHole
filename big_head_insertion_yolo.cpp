...
// *************** LIBRERIE
  ...
  #include "utils.hpp"
  #include "utils_detector_yolo.hpp"
...
// *************** DICHIARAZIONE FUNZIONI
 ...
 std::array<double, 16> move(std::string &robot_ip, Eigen::Vector3d final_point, Eigen::MatrixXd final_matrix,Eigen::Vector3d nH, double ti, double tf, double vi, double vf, int seg, int num_seg,double t_ass);
...
int main(int argc, char** argv) {
  ...
  // *************** VERSORE FORO
    ...
    std::string filepath14 = path + "/foro_stimato_nella_ricostruzione.txt"; 
    FILE* file14_ = fopen(filepath14.c_str(), "a");
    fprintf(file14_, "Foro stimato prima di YOLO:\n");
    fprintf(file14_, "%f %f %f\n", p_target[0], p_target[1], p_target[2]);
    fclose(file14_);
  ...
  //
   Eigen::Vector3d p_temp; 
    ...
    // p_target e' in terna mondo e rappresenta il punto in cui andrà l'end-effector
    cout << "Approach to hole with camera" << endl;
    //vado nel punto stabilito con un delta su z di 30 cm
    double delta_f = -0.25;
    if(bhToInsert == 3){
      delta_f = -0.25;
    }
    p_temp = p_target + R_target * Eigen::Vector3d(0,0,delta_f);
    //sposto il punto per fare in modo che sia la camera a trovarsi in p_temp
    //p_temp = p_temp - R_target * Eigen::Vector3d(0.05834485203,-0.03476564525,-0.06760482074);
    p_temp = p_temp - R_target * Eigen::Vector3d(0.05834485203, -0.03476564525, -0.05430977191);
    std::array<double, 16> pose;
    pose = move(robot_ip,  Eigen::Vector3d(p_temp[0], p_temp[1], p_temp[2]),R_target, nH, 0, tempo, 0, 0,Approach,num_seg,t_ass);

    cout << "Actual pose\n"<< pose[12] << " " << pose[13] << " " << pose[14] << " "<< endl;
    // running neural network 
    Eigen::Vector3d p_hole_adj;
    F <<  pose[0], pose[4], pose[8], pose[12],
    ...
    fprintf(file15_, "wTe = [%f %f %f %f\n%f %f %f %f\n%f %f %f %f\n0 0 0 1];\n",
             F(0,0), F(0,1), F(0,2), F(0,3),
             F(1,0), F(1,1), F(1,2), F(1,3),
             F(2,0), F(2,1), F(2,2), F(2,3));
    fclose(file15_);

    p_hole_adj = detectHole(F, path);

    fprintf(file13_, "%f %f %f ", p_hole_adj[0], p_hole_adj[1], p_hole_adj[2]);
    ...
    while(seg<=4){
      if(seg==Approach){
        ...
        p_temp = p_hole_adj + R_target * Eigen::Vector3d(0,0,-0.05);
        ...
        p_temp = p_hole_adj + R_target * Eigen::Vector3d(0,0,0.02); //-0.018);
  ...
  // ***************  ROBOT CONTROL             
    ...
    // *************** INVERSIONE CINEMATICA
      ...
      if (time > t_f+t_ass-0.1 && seg!=Insertion){//|| (seg==2 && time>10)){
          q_dot_0 << 0,0,0,0,0,0,0;             
      }else if((seg==Insertion && pStop[2] > 0.004) || time > t_f+30 /*|| (seg==Insertion && pH[2] > -0.01) */) { ////initial_pose[14]<z_contact-0.005 ){
    ...
    // *************** CONDIZIONE DI USCITA DAL CICLO DI CONTROLLO
      if(seg == Insertion  && bhInsertion==true  || time > t_f+t_ass+30 ){ // || seg==Insertion && pH[2]>0.008  time >= 5*t_f+t_ass
        ...
        //fprintf(file13_, "%f %f %f %f %f\n",tcontact, tcontactEnd-tcontact, holeMeasured[0], holeMeasured[1], holeMeasured[2]);
