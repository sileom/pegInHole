...
int main(int argc, char** argv) {
  if (argc != 4) {
    std::cerr << "Usage: " << argv[0] << " <robot-hostname> " << "<input file path> " << "<path to save> " << std::endl;
    return -1;
  }
  ...
  //
    ...
    move(robot_ip,  Eigen::Vector3d(P(Insertion,0), P(Insertion,1),P(Insertion,2)), Rf_vec[Insertion], nH, 0, tempo, 0, 0,Insertion,num_seg,t_ass);
        if(bhInsertion==true){
...
int move(std::string &robot_ip, Eigen::Vector3d final_point, Eigen::MatrixXd final_matrix,Eigen::Vector3d nH, double ti, double tf, double vi, double vf, int seg,int num_seg,double t_ass){
  ...
  // ***************  ROBOT CONTROL             
    ...
    // *************** FILTRAGGIO DELLE FORZE LETTE DALL'OSSERVATORE
       Eigen::VectorXd ff = filter(f_n_1, f_ext, alpha_filt);
    ...
    // *************** AMMETTENZA IN TERNA END-EFFECTOR
      ...
      // MOTO DI RICERCA DEL FORO
       ...
       if(seg == Insertion && contact(seg,ffr1)==true  && searchC==false && InsertionVar==false){// 
           ...
           InsertionVar=true;           
           tcontact=time;
           z_contact=initial_pose[14];
        //std::cout <<"tcontact"<< " "<< tcontact << endl;
       ...
       if(seg == Insertion && searchC==true){
         ...
         ppd[0]=3*ax*cos(3*(time-tcontact));
         ppd[1]=4*ay*cos(4*(time-tcontact));
         
         ppd=Rr*ppd;
         //std::cout <<"Zeta"<< " "<< initial_pose[14] << endl;
        //std::cout <<"Zcontact"<< " "<< z_contact << endl; 
         if(initial_pose[14]<z_contact-0.001){
             searchC=false;
             std::cout <<"serch=false"<< endl;
         }
      ...
      // AMMETTENZA IN TERNA BASE

      /*Eigen::MatrixXd A(6,6);
      Eigen::VectorXd diag6(6);
      diag6 << 0,0,0,0,0,0;
      A= getDiagonalMatrix(6, diag6);
      
      Eigen::MatrixXd D(6,6);
      D= getDiagonalMatrix(6, diag6);
      A.block<3,3>(0,0)=Ap;
      A.block<3,3>(3,3)=Af;
      D.block<3,3>(0,0)=Dp;
      D.block<3,3>(3,3)=Df;     

      ff[3]=ff[3];ff[4]=-ff[4];ff[5]=-ff[5];
      xpp_r = xppd + A.inverse()*(D*(xpd - xp_r)+ff);
      
      xp_r = xp_r + xpp_r*period.toSec();
      x_r = x_r + xp_r*period.toSec();
      Eigen::Vector3d eulp;
      eulp <<  xp_r[3], xp_r[4], xp_r[5];
      Eigen::Vector3d eul;
      eul <<  x_r[3], x_r[4], x_r[5];
      //eul <<  xpd[3], xpd[4], xpd[5];
      
      omegar = Ti(eul) * eulp;*/

    // *************** SCELTA DELLE VARIABILI DA MANDARE AL ROBOT des=1 => variabili desiderate al robot
      int des=0;
      if (des==0 && (seg==Insertion || seg==Bonding || seg==1)){
      ...
      //x_dot_d << xp_r[0], xp_r[1], xp_r[2], omegar[0], omegar[1], omegar[2];
    ...
    // *************** INVERSIONE CINEMATICA
      ...
      if (time > t_f+t_ass-0.1 && seg!=Insertion){//|| (seg==2 && time>10)){
          q_dot_0 << 0,0,0,0,0,0,0;             
      }else if(seg==Insertion && initial_pose[14]<z_contact-0.004 ){
            q_dot_0 << 0,0,0,0,0,0,0;             
            stop_cycles++;
            std::cout <<"stop_cycles"<< " " << stop_cycles << endl;
      }
      if(stop_cycles>100){
            bhInsertion=true;
    ...
    // *************** CONDIZIONE DI USCITA DAL CICLO DI CONTROLLO
      if(seg == Insertion  && bhInsertion==true  ){ //time >= 5*t_f+t_ass
        std::cout << std::endl << "Insertion completed" << std::endl;
